...
int agregarPelicula(eMovie peliculas[], int tam)
{
    ...
    if((indice=espacioLibrePelicula(peliculas,tam))!=-1)
    {
        ...
        char auxTitulo[20], auxDescripcion[50], auxGenero[20];
        ...
        scanf("%d",&auxPuntaje);
        if (validarCadena(auxTitulo,20) && validarCadena(auxDescripcion,50) && validarCadena(auxGenero,20)
...
int menuPeliculas (eMovie peliculas[], int tam)
{
    ...
    for(i=0; i<tam; i++)
    {
        ...
        {
            printf("%d- %s",(i+1),peliculas[i].titulo);
        }
...
int buscarPelicula (eMovie peliculas[], int tam, int id)
{
    ...
    for (i=0; i<tam && flag==0; i++)
    {
        if(peliculas[i].id==id && peliculas[i].id!=0)
        {
            indice=i;
            flag=1;
        }
...
void generarPagina(eMovie peliculas[], char nombre[], int tam)
{
    ...
    char post_articulo_1[]="<!-- jQuery --><script src='js/jquery-1.11.3.min.js'></script><!-- Bootstrap Core JavaScript --><script src='js/bootstrap.min.js'></script><!-- IE10 viewport bug workaround --><script src='js/ie10-viewport-bug-workaround.js'></script><!-- Placeholder Images --><script src='js/holder.min.js'></script></body></html>";
    ...
    else
    {
        ...
        cantidadEscrita=fprintf(pArchivo,post_articulo_1);
        if(cantidadEscrita<0)
        {
            printf("Error al escribir archivo.\n");
        }
